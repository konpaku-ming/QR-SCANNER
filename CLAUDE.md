...
src/
  ...
  popup/                   # Extension popup (scan/clear/clipboard controls)
    popup.html
    popup.js
...
- **Library loading**: All libraries (zxing-wasm.iife.js, qr-engine.js, qr-decoder.js, jsQR.js) are loaded automatically by `manifest.json` `content_scripts` in the isolated world. `background.js` no longer performs any `executeScript` dynamic injection.
...
- **Manifest permissions**: `activeTab`, `scripting`, `storage`, `contextMenus`, `tabs`, `clipboardRead`, and `<all_urls>` host permission. `storage` and `tabs` are declared but largely unused in the current implementation.
- **TODOs in codebase**: Options page persistence and `background-image` / Canvas scanning are still unimplemented (see `Plan.md` §4–5). Scan history, SPA MutationObserver re-scanning, clipboard scan, zxing-wasm integration, and whole-image multi-QR detection are now implemented.
...
- **Package for distribution**: Zip the repository root (excluding `.git`, `.claude/`, `node_modules/`, and ignored files) and upload to the Edge Add-ons portal.
...
- **Node.js unit tests**: `node tests/node-unit-test.js` — tests pure logic (truncate, coordinate mapping, image filtering, jsQR loading, qr-decoder pure functions) with zero browser dependencies.
